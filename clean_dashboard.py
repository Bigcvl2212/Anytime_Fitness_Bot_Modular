...
# Note: Removed adding current directory to avoid conflicts with src/clean_dashboard.py
...
# Now import the ClubOS APIs with proper error handling
ClubOSRealCalendarAPI = None
iCalClubOSParser = None
ClubOSEventDeletion = None
ClubOSFreshDataAPI = None

try:
    from src.clubos_real_calendar_api import ClubOSRealCalendarAPI
    logger.info("✅ ClubOS Real Calendar API imported successfully")
except ImportError as e:
    logger.error(f"❌ Failed to import ClubOS Real Calendar API: {e}")

try:
    from src.ical_calendar_parser import iCalClubOSParser
    logger.info("✅ iCal Calendar Parser imported successfully")
except ImportError as e:
...
try:
    ...
    logger.error(f"❌ Failed to import ClubOS Event Deletion: {e}")

try:
    from src.clubos_fresh_data_api import ClubOSFreshDataAPI
    logger.info("✅ ClubOS Fresh Data API imported successfully")
except ImportError as e:
    logger.error(f"❌ Failed to import ClubOS Fresh Data API: {e}")

# Log successful imports
logger.info("✅ All required modules processed")
...
logger.info(f"📁 prospects.html exists: {os.path.exists(os.path.join(templates_dir, 'prospects.html'))}")
...
clubos_fresh_data_api = ClubOSFreshDataAPI() if ClubOSFreshDataAPI else None
...
class TrainingPackageCache:
    ...
    def _fetch_fresh_funding_data(self, member_id: str, member_name: str) -> dict:
        ...
        try:
            # Use the simple working get_member_payment_status method
            ...
            if payment_status:
                ...
                funding_data = {
                    ...
                    'amount_remaining': 0.0,  # We don't have exact amounts from simple method
                ...
                # IMPORTANT: Save this person as a training client in the database
                self._save_training_client_to_database(member_id, member_name, payment_status)
    ...
    def _save_training_client_to_database(self, member_id: str, member_name: str, payment_status: str):
        """Save a training client to the training_clients table when we discover them"""
        try:
            conn = sqlite3.connect(db_manager.db_path)
            cursor = conn.cursor()
            
            # Check if this training client already exists
            cursor.execute("""
                SELECT id FROM training_clients 
                WHERE clubos_member_id = ? OR member_name = ?
                LIMIT 1
            ...
            if existing:
                ...
                cursor.execute("""
                    ...
                    SET payment_status = ?, last_updated = ?
                    WHERE id = ?
                """, (payment_status, datetime.now().isoformat(), existing[0]))
                logger.info(f"🔄 Updated existing training client: {member_name}")
            else:
                # Insert new training client
                cursor.execute("""
                    INSERT INTO training_clients (
                        clubos_member_id, member_name, payment_status, 
                        sessions_remaining, last_updated, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    member_id, member_name, payment_status,
                    0,  # Default sessions remaining
                    datetime.now().isoformat(),
                    datetime.now().isoformat()
                ))
                logger.info(f"✅ Added new training client to database: {member_name}")
            
            conn.commit()
            conn.close()
            
        except Exception as e:
            logger.error(f"❌ Error saving training client {member_name} to database: {e}")
...
def import_fresh_clubhub_data():
    ...
    try:
        ...
        print("✅ ClubHub authenticated - importing fresh data from API...")
        ...
        print("📥 Loading members from ClubHub API...")
        while True:
            ...
            if page > 200:  # Safety limit
                print(f"⚠️ Reached page limit (200) for members")
                break
        
        print(f"📥 Retrieved {len(all_members)} members from ClubHub")
        
        # Get ALL prospects from ClubHub - this is what was working before
        all_prospects = []
        page = 1
        
        print("📥 Loading ALL prospects from ClubHub API...")
        while True:
            try:
                # Use the working method from comprehensive_data_pull_WORKING.py
                prospects_url = f"https://clubhub-ios-api.anytimefitness.com/api/clubs/1156/prospects"
                params = {"page": page, "pageSize": 100}
                
                response = requests.get(prospects_url, headers=client.headers, params=params, timeout=30)
                ...
                if response.status_code == 200:
                    ...
                    print(f"  📄 Prospects page {page}: {len(prospects_data)} found")
                    ...
                    page += 1
                    if page > 200:  # Safety limit
                        print(f"⚠️ Reached page limit (200) for prospects")
                        break
                else:
                    print(f"❌ Prospects API failed: {response.status_code}")
                    break
                    
            except Exception as e:
                print(f"❌ Error fetching prospects page {page}: {e}")
                break
...
# --- Startup warm-up: populate training_clients from ClubOS in the background ---
def _populate_training_clients_from_clubos(save_to_db: bool = True) -> int:
    """Fetch ClubOS training clients (assignees) and optionally persist to DB.

    Returns the number of clients processed/saved.
    """
    try:
        # Ensure ClubOS Training API is authenticated
        if not clubos_training_api.authenticated:
            logger.info("🔐 Authenticating ClubOS Training API for warm-up…")
            if not clubos_training_api.authenticate():
                logger.warning("❌ ClubOS Training API authentication failed (warm-up)")
                return 0

        # Prefer a fresh pull; fall back to cached session fetch
        assignees = (
            clubos_training_api.fetch_assignees(force_refresh=True)
            or clubos_training_api.fetch_assignees(force_refresh=False)
        )
        if not assignees:
            logger.warning("⚠️ No training assignees returned from ClubOS during warm-up")
            return 0

        if not save_to_db:
            return len(assignees)

        # Persist to DB (lightweight fields to avoid slow per-client calls)
        conn = sqlite3.connect(db_manager.db_path)
        cursor = conn.cursor()
        # Clear stale records so counts reflect live state
        cursor.execute("DELETE FROM training_clients")
        ...
        for a in assignees:
            try:
                ...
                if not member_id:
                    continue
                member_name = a.get('name', 'Unknown')
                email = a.get('email') or 'N/A'
                phone = a.get('phone') or 'N/A'

                # Defer expensive agreement lookups; keep it fast at startup
                ...
                cursor.execute(
                    ...
                    (
                        ...
                        payment_status, now_iso, now_iso,
                    ),
                )
                saved += 1
            except Exception as ie:
                logger.warning(f"⚠️ Skipping assignee due to insert error: {ie}")

        conn.commit()
        conn.close()
        logger.info(f"✅ Warm-up saved {saved} training clients to database")
        return saved

    except Exception as e:
        logger.error(f"❌ Warm-up error while populating training clients: {e}")
        return 0


def _warmup_training_clients_background():
    """Run training clients warm-up in background if table is empty."""
    try:
        ...
        if existing > 0:
            logger.info(f"⏭️ Skipping warm-up; training_clients already populated ({existing})")
            return

        logger.info("🔧 Warm-up: fetching training clients from ClubOS in background…")
        count = _populate_training_clients_from_clubos(save_to_db=True)
        logger.info(f"📦 Warm-up complete: {count} training clients saved")
    except Exception as e:
        logger.error(f"❌ Warm-up background task failed: {e}")


def _start_startup_warmups():
    try:
        t = threading.Thread(
            target=_warmup_training_clients_background,
            name="warmup-training-clients",
            daemon=True,
        )
        t.start()
    except Exception as e:
        logger.error(f"❌ Failed to start warm-up thread: {e}")


# Kick off background warm-ups after initial ClubHub import
_start_startup_warmups()

# --- Periodic refresh jobs (optional improvements) ---
def _periodic_training_clients_refresh(interval_minutes: int = 60):
    """Background loop to refresh training_clients from ClubOS periodically."""
    try:
        logger.info(f"⏲️ Starting periodic training clients refresh every {interval_minutes} minutes…")
        while True:
            try:
                count = _populate_training_clients_from_clubos(save_to_db=True)
                logger.info(f"🔁 Periodic refresh saved {count} training clients")
            except Exception as ie:
                logger.warning(f"⚠️ Periodic training clients refresh error: {ie}")
            # Sleep until next interval
            time.sleep(max(5, int(interval_minutes) * 60))
    except Exception as e:
        logger.error(f"❌ Periodic refresh thread crashed: {e}")


def _daily_funding_cache_refresh(interval_hours: int = 24):
    """Background loop to refresh funding cache for all training clients daily."""
    try:
        logger.info(f"⏲️ Starting funding cache refresh every {interval_hours} hours…")
        while True:
            try:
                result = training_package_cache.refresh_all_funding_cache()
                logger.info(
                    f"💾 Funding cache refresh result: success={result.get('success', False)} updated={result.get('success_count', 0)}/{result.get('total_clients', 0)}"
                )
            except Exception as ie:
                logger.warning(f"⚠️ Daily funding cache refresh error: {ie}")
            # Sleep until next interval
            time.sleep(max(300, int(interval_hours) * 3600))
    except Exception as e:
        logger.error(f"❌ Funding cache refresh thread crashed: {e}")


def _start_periodic_jobs():
    """Kick off periodic background jobs (hourly client list, daily funding cache)."""
    try:
        threading.Thread(
            target=_periodic_training_clients_refresh,
            name="periodic-training-clients-refresh",
            daemon=True,
        ).start()
    except Exception as e:
        logger.error(f"❌ Failed to start periodic training clients refresh: {e}")
    try:
        threading.Thread(
            target=_daily_funding_cache_refresh,
            name="daily-funding-cache-refresh",
            daemon=True,
        ).start()
    except Exception as e:
        logger.error(f"❌ Failed to start daily funding cache refresh: {e}")
...
def api_training_payment_status():
    ...
    try:
        ...
        # Use the simple working get_member_payment_status method
        logger.info(f"🔍 Getting simple payment status for member: {member_id}")
        
        try:
            payment_status = clubos_training_api.get_member_payment_status(member_id)
            
            if payment_status:
                # Determine status based on simple payment status
                if payment_status == 'Past Due':
                    status_text = "Past Due"
                    status_class = 'danger'
                    status_icon = 'fas fa-exclamation-circle'
                    amount_owed = 1.0  # Placeholder - we don't have exact amounts from simple method
                ...
                # IMPORTANT: Save this person as a training client in the database
                training_package_cache._save_training_client_to_database(member_id, participant_name or f"Member {member_id}", payment_status)
                
                            # IMPORTANT: Save this person as a training client in the database
            try:
                training_package_cache._save_training_client_to_database(member_id, participant_name or f"Member {member_id}", payment_status)
                logger.info(f"✅ Saved training client to database: {participant_name or f'Member {member_id}'}")
            except Exception as e:
                logger.warning(f"⚠️ Could not save training client to database: {e}")
            ...
            return jsonify({
                ...
                'funding': {
                    ...
                    'source': 'simple_payment_status'
                }
            })
        else:
            logger.warning(f"⚠️ No payment status returned for member {member_id}")
            return jsonify({'success': True, 'funding': {
                'status_text': 'Unknown', 'status_class': 'secondary', 'status_icon': 'fas fa-question-circle'
                }})
                
        except Exception as e:
            logger.error(f"❌ Error getting payment status for {member_id}: {e}")
...
def api_test_known_agreement():
    """Test endpoint to validate agreement API methods with a known working agreement ID."""
    try:
        data = request.get_json(silent=True) or request.form or request.args or {}
        agreement_id = data.get('agreement_id', '1522516')
        ...
        logger.error(f"❌ Error testing known agreement: {e}")
...
def prospects_page():
    ...
    logger.info("🏠 Prospects page accessed")
    logger.info(f"📁 App template folder: {app.template_folder}")
    logger.info(f"📄 Looking for prospects.html in: {app.template_folder}")
    
    template_path = os.path.join(app.template_folder, 'prospects.html')
...
def prospect_profile(prospect_id):
    ...
    try:
        ...
        prospect = dict(zip(columns, prospect_data)) if prospect_data else {}
        
        # Enhance with additional data if needed
        prospect_info = {
            'id': prospect.get('id', prospect_id),
            'guid': prospect.get('guid', ''),
            'full_name': prospect.get('full_name', f"{prospect.get('first_name', '')} {prospect.get('last_name', '')}".strip()),
    ...
    try:
        ...
        for assignee in clubos_assignees:
            try:
                ...
                # Skip slow API calls during initial load for fast page performance
                # Payment status and package agreements will be fetched on-demand
                payment_status = 'Unknown'  # Will be fetched when needed
                agreements = []  # Will be fetched when user clicks on training client card
                
                # Extract primary agreement info (first active agreement)
                primary_agreement = None
                if agreements:
                    primary_agreement = agreements[0]  # Take first agreement as primary
                
                package_name = primary_agreement.get('package_name', 'No package assigned') if primary_agreement else 'No package assigned'
                trainer_name = primary_agreement.get('trainer_name', 'Jeremy Mayo') if primary_agreement else 'Jeremy Mayo'
                sessions_remaining = primary_agreement.get('sessions_remaining', 'N/A') if primary_agreement else 'N/A'
                agreement_amount = primary_agreement.get('amount', 'N/A') if primary_agreement else 'N/A'
...
def api_invoices_create():
    """API endpoint to create a Square invoice - matches template expectations."""
    data = request.get_json()
    member_id = data.get('member_id')
    amount = data.get('amount')
    description = data.get('description', 'Payment Due')

    if not all([member_id, amount]):
        return jsonify({'success': False, 'error': 'Missing required invoice data (member_id and amount).'}), 400
    ...
    try:
        ...
        # Try training_clients table first
        cursor.execute("SELECT member_name FROM training_clients WHERE member_id = ? OR clubos_member_id = ?", (member_id, member_id))
        ...
        if training_client:
            ...
            if member:
                ...
                member_name = f"Member {member_id}"  # Fallback
...
def api_invoices_batch():
    """API endpoint to create batch Square invoices."""
    data = request.get_json()
    invoice_type = data.get('type')  # 'training_clients' or 'members'
    filter_type = data.get('filter')  # 'past_due' or 'all'
    selected_clients = data.get('selected_clients', [])

    if not all([invoice_type, selected_clients]):
        return jsonify({'success': False, 'error': 'Missing required batch invoice data.'}), 400

    if not SQUARE_AVAILABLE:
        return jsonify({'success': False, 'error': 'Square payment service is not available.'}), 503

    try:
        logger.info(f"🔄 Starting batch invoice creation for {len(selected_clients)} {invoice_type}")
        ...
        for client_id in selected_clients:
            try:
                if invoice_type == 'training_clients':
                    # Get training client data
                    cursor.execute("""
                        SELECT member_name, email, funding_cache 
                        FROM training_clients 
                    ...
                    if client:
                        ...
                        # Calculate invoice amount from funding cache
                        funding_cache = json.loads(client['funding_cache'] or '{}')
                        amount = max(funding_cache.get('amount_remaining', 50.0), 5.0)  # Minimum $5
                        description = "Training Package Payment"
                    else:
                        failed_invoices.append({'client_id': client_id, 'error': 'Training client not found'})
                        continue
                        
                elif invoice_type == 'members':
                    # Get member data
                    cursor.execute("""
                        SELECT first_name, last_name, email, amount_past_due, amount_of_next_payment
                        FROM members 
                        WHERE id = ?
                    """, (client_id,))
                    member = cursor.fetchone()
                    ...
                    if member:
                        ...
                        # Use past due amount + late fee, or next payment, with minimum $5
                        amount = max(past_due + (25.0 if past_due > 0 else 0) or next_payment or 50.0, 5.0)
                        description = f"Membership Payment{' - Past Due' if past_due > 0 else ''}"
                    else:
                        failed_invoices.append({'client_id': client_id, 'error': 'Member not found'})
                        continue
                
                # Create Square invoice with correct parameters (name, email, amount, description)
                invoice_result = create_square_invoice(member_name, member_email, float(amount), description)
                ...
                if invoice_url:
                    ...
                    logger.info(f"✅ Invoice created for {member_name}: ${float(amount):.2f}")
                else:
                    failed_invoices.append({
                        'client_id': client_id, 
                        'member_name': member_name,
                        'amount': float(amount),
                        'error': 'Square invoice creation failed'
                    })
                    logger.error(f"❌ Failed to create invoice for {member_name} (${float(amount):.2f})")
                    
            except Exception as e:
                logger.error(f"❌ Error processing {client_id}: {e}")
                failed_invoices.append({'client_id': client_id, 'error': str(e)})
        
        conn.close()
        
        logger.info(f"🎉 Batch invoicing completed: {len(successful_invoices)} successful, {len(failed_invoices)} failed")
        ...
        return jsonify({
            ...
            'message': f'Batch invoicing completed: {len(successful_invoices)} successful, {len(failed_invoices)} failed'
        })
        
    except Exception as e:
        logger.error(f"Error creating batch Square invoices: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
