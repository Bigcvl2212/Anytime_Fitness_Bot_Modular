...
def enhanced_startup_sync(app, selected_clubs_override: List[str] = None, multi_club_enabled: bool = True) -> Dict[str, Any]:
    ...
    try:
        if multi_club_enabled:
            ...
            # CRITICAL FIX: Save fresh data to database
            logger.info("💾 Saving fresh ClubHub data to database...")
            try:
                ...
                if members_data:
                    logger.info(f"💾 Saving {len(members_data)} members to database...")
                    db_manager.save_members_to_db(members_data)
                
                # Save prospects to database  
                prospects_data = combined_data.get('prospects', [])
                if prospects_data:
                    logger.info(f"💾 Saving {len(prospects_data)} prospects to database...")
                    db_manager.save_prospects_to_db(prospects_data)
                
                # Save training clients to database
                training_data = combined_data.get('training_clients', [])
                if training_data:
                    logger.info(f"💾 Saving {len(training_data)} training clients to database...")
                    db_manager.save_training_clients_to_db(training_data)
                
                logger.info("✅ Fresh ClubHub data saved to database successfully!")
                
            except Exception as db_error:
                logger.error(f"❌ Error saving fresh data to database: {db_error}")
            ...
            try:
                ...
                if members:
                    ...
                    db_manager.save_members_to_db(members)
                
                # Save prospects to database  
                if prospects:
                    logger.info(f"💾 Saving {len(prospects)} prospects to database...")
                    db_manager.save_prospects_to_db(prospects)
                
                # Save training clients to database
                if training_clients:
                    logger.info(f"💾 Saving {len(training_clients)} training clients to database...")
                    db_manager.save_training_clients_to_db(training_clients)
