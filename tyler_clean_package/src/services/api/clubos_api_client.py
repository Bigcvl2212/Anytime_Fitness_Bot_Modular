...
# Try to import debug_page_state, but don't fail if not available
try:
    from utils_legacy.debug_helpers import debug_page_state
except ImportError:
    # Fallback function if debug_helpers is not available
...
class ClubOSAPIAuthentication:
    ...
    def login(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            # Step 3: Check for club selection redirect (common ClubOS pattern)
            if "ChooseClub" in auth_response.url or "chooseclub" in auth_response.text.lower():
                print("   🏢 Club selection required - handling automatically...")
                success = self._handle_club_selection(auth_response)
                if not success:
                    print("   ❌ Club selection failed")
                    return False
            
            # Step 4: Extract session information from cookies (working pattern)
    ...
    def _handle_club_selection(self, response: requests.Response) -> bool:
        """Handle club selection page after login"""
        try:
            from bs4 import BeautifulSoup
            soup = BeautifulSoup(response.text, 'html.parser')
            
            # Look for club selection form or links
            club_forms = soup.find_all('form', action=lambda x: x and 'chooseclub' in x.lower())
            club_links = soup.find_all('a', href=lambda x: x and 'chooseclub' in x.lower())
            
            # Try to find a club to select (usually the first one)
            club_id = None
            
            # Method 1: Look for club forms with club IDs
            for form in club_forms:
                club_input = form.find('input', {'name': re.compile(r'club.*id', re.I)})
                if club_input and club_input.get('value'):
                    club_id = club_input.get('value')
                    break
            
            # Method 2: Look for club links with club IDs
            if not club_id:
                for link in club_links:
                    href = link.get('href', '')
                    club_match = re.search(r'club.*?(\d+)', href, re.I)
                    if club_match:
                        club_id = club_match.group(1)
                        break
            
            # Method 3: Look for any club selection elements
            if not club_id:
                club_elements = soup.find_all(['input', 'select', 'button'], 
                                            {'name': re.compile(r'club', re.I)})
            ...
            if not club_id:
                print("   ⚠️ Could not find club ID in selection page")
                # Try to submit the first available form without specific club ID
                if club_forms:
                    form = club_forms[0]
                    action = form.get('action')
                    if action:
                        print("   🔄 Trying to submit first available club form...")
                        form_data = {}
                        ...
                        club_response = self.session.post(
                            urljoin(self.base_url, action),
                            data=form_data,
                            timeout=30
                        )
                        
                        if club_response.ok and "login" not in club_response.url.lower():
                            print("   ✅ Club selection completed successfully")
                            return True
                
                return False
            
            print(f"   🏢 Found club ID: {club_id}, selecting...")
            ...
            # Try common club selection endpoints
            club_endpoints = [
                '/action/ChooseClub',
                '/action/chooseclub', 
                '/ChooseClub',
            ...
            for endpoint in club_endpoints:
                try:
                    ...
                    # Check if club selection was successful (redirect away from login/club selection)
                    if club_response.ok and "login" not in club_response.url.lower() and "chooseclub" not in club_response.url.lower():
                        print(f"   ✅ Club selection successful via {endpoint}")
                        return True
                        
                except Exception as e:
                    print(f"   ⚠️ Club selection attempt via {endpoint} failed: {e}")
                    continue
            
            print("   ❌ All club selection attempts failed")
            return False
            
        except Exception as e:
            print(f"   ❌ Error handling club selection: {e}")
            return False
...
class ClubOSAPIClient:
    ...
    def discover_api_endpoints(self) -> Dict[str, Any]:
        """
        Use pre-configured API endpoints instead of discovery
        
        Returns:
            Dict containing known working endpoints
        """
        print("✅ Using known working ClubOS endpoints")
        
        # Return the pre-configured endpoints we know work
        return {
            "dashboard": ["/action/Dashboard/messages"],
            "calendar": ["/action/Calendar"],
            "members": ["/action/Dashboard/view"],
            "known_working": True
        }
...
def create_clubos_api_client(username: str, password: str) -> Optional[ClubOSAPIClient]:
    ...
    try:
        ...
        if auth_service.login(username, password):
            ...
            # Skip endpoint discovery - use known working endpoints
            print("✅ Using pre-configured ClubOS API endpoints")
