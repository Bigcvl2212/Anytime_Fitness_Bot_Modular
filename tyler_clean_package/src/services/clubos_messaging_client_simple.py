...
class ClubOSMessagingClient:
    ...
    def send_bulk_campaign_optimized(self, member_data_list: List[Dict[str, Any]] = None, member_ids: List[str] = None, message: str = "", message_type: str = "sms", max_workers: int = 5) -> Dict[str, Any]:
        """
        Optimized bulk campaign that preserves exact API call sequence but runs concurrently.
        
        Key optimization: Each worker gets its own authentication session and runs the complete
        workflow (auth → delegate → message) independently, preserving the finicky ClubOS order.
        
        Args:
            member_data_list: List of member data dictionaries
            member_ids: List of member IDs (alternative to member_data_list)
            message: Message text to send
            message_type: "sms" or "email"
            max_workers: Number of concurrent workers (default 5 for safety)
        """
        # Handle both parameter styles
        if member_data_list:
            members_to_process = member_data_list
        ...
        logger.info(f"🚀 Starting OPTIMIZED bulk campaign to {total_count} members with {max_workers} workers")
        start_time = time.time()
        
        # Thread-safe counters
        success_counter = threading.Lock()
        failed_counter = threading.Lock()
        error_list_lock = threading.Lock()
        messages_lock = threading.Lock()
        
        def process_member(member_data, worker_id):
            """
            Process a single member with complete workflow in isolated session.
            Preserves exact API call sequence: authenticate → delegate → send message
            """
            try:
                member_id = member_data.get('member_id') or member_data.get('prospect_id') or str(member_data.get('id', ''))
                member_name = member_data.get('name') or member_data.get('full_name', f'Member {member_id}')
                ...
                # Create isolated messaging client for this worker
                worker_client = ClubOSMessagingClient(self.username, self.password)
                
                # Step 1: Authenticate (preserves exact sequence)
                if not worker_client.authenticate():
                    with failed_counter:
                        results['failed'] += 1
                    with error_list_lock:
                        results['errors'].append(f"Authentication failed for {member_name}")
                    return False
                ...
                if message_type == 'sms' and member_data.get('fallback_to_email'):
                    actual_channel = 'email'
                    logger.info(f"🔄 Worker {worker_id}: Using email fallback for {member_name} (no phone number)")
                
                logger.info(f"📨 Worker {worker_id}: Sending {actual_channel} to {member_name} (ID: {member_id})")
                
                # Step 2: Send message (includes delegation internally - preserves exact sequence)
                success = worker_client.send_message(
                    member_id=member_id,
                    message_text=message,
                    channel=actual_channel,
                    member_data=member_data
                ...
                if success:
                    ...
                    logger.info(f"✅ Worker {worker_id}: Message sent successfully to {member_name}")
                    return True
                else:
                    with failed_counter:
                        results['failed'] += 1
                    with error_list_lock:
                        results['errors'].append(f"Failed to send to {member_name} (ID: {member_id})")
                    logger.error(f"❌ Worker {worker_id}: Failed to send to {member_name}")
                    return False
        ...
        # Process members concurrently with ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            future_to_member = {
                executor.submit(process_member, member_data, i % max_workers): member_data 
                for i, member_data in enumerate(members_to_process)
            }
            ...
            for future in as_completed(future_to_member):
                ...
                member_name = member_data.get('name') or member_data.get('full_name', 'Unknown')
                
                try:
                    success = future.result()
                    logger.info(f"📊 Progress: {completed}/{total_count} completed - {member_name}")
                except Exception as e:
                    logger.error(f"❌ Task failed for {member_name}: {e}")
                    with failed_counter:
                        results['failed'] += 1
                    with error_list_lock:
                        results['errors'].append(f"Task failed for {member_name}: {str(e)}")
        
        # Calculate performance metrics
        end_time = time.time()
        processing_time = end_time - start_time
        results['processing_time'] = round(processing_time, 2)
        results['messages_per_minute'] = round((results['successful'] / processing_time) * 60, 1) if processing_time > 0 else 0
        
        logger.info(f"🚀 OPTIMIZED Campaign completed:")
        logger.info(f"   📊 {results['successful']}/{results['total']} successful ({results['successful']/results['total']*100:.1f}%)")
        logger.info(f"   ⏱️  Processing time: {results['processing_time']} seconds")
        logger.info(f"   🚀 Speed: {results['messages_per_minute']} messages/minute")
