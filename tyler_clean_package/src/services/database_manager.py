...
class DatabaseManager:
    ...
    def __init__(self, db_path=None):
        ...
        try:
            ...
            # Don't re-raise the exception - let the app continue
    ...
    def get_connection(self):
        ...
        if self.db_type == 'postgresql':
            try:
                ...
                logger.error(f"   Host: {self.postgres_config['host']}")
                logger.error(f"   Port: {self.postgres_config['port']}")
                logger.error(f"   Database: {self.postgres_config['dbname']}")
                logger.error(f"   User: {self.postgres_config['user']}")
                raise e
        else:
            import sqlite3
    ...
    def get_member_by_id(self, member_id: str) -> Optional[Dict]:
        """Get member by ID (prospect_id)"""
        conn = self.get_connection()
        cursor = conn.cursor()
        
        try:
            if self.db_type == 'postgresql':
                query = "SELECT * FROM members WHERE prospect_id = %s"
            else:
                query = "SELECT * FROM members WHERE prospect_id = ?"
    ...
    def update_member(self, member_id: str, member_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for field, db_column in allowed_fields.items():
                if field in member_data:
                    ...
                    values.append(member_data[field])
            
            if not update_fields:
                logger.warning(f"No valid fields to update for member {member_id}")
                return False
            ...
            logger.info(f"Updating member {member_id} with fields: {list(member_data.keys())}")
        ...
        try:
            ...
            try:
                ...
                if not username or not password:
                    logger.warning("⚠️ ClubOS credentials not found, returning empty messages")
                    return []
                ...
                all_senders = clubos_service.get_all_message_senders(limit=50)
                
                messages = []
                if all_senders:
                    logger.info(f"✅ Found {len(all_senders)} message senders")
                    
                    # Get messages from each sender
                    for sender in all_senders:
                        logger.info(f"📨 Getting messages from: {sender}")
                        try:
                            conversation = clubos_service.scrape_conversation_for_contact(sender)
                            
                            if conversation and len(conversation) > 0:
                                # Get the most recent messages from this sender
                                recent_messages = conversation[-3:]  # Last 3 messages per sender
                                
                                for i, msg in enumerate(recent_messages):
                                    message_data = {
                                        'id': f"clubos_{sender.replace(' ', '_')}_{i}",
                                        'member_name': sender,
                                        ...
                                        'unread': msg.get('sender') != 'j.mayo' and i >= len(recent_messages) - 1  # Mark last message as unread
                                    }
                                    messages.append(message_data)
                            else:
                                logger.info(f"ℹ️ No conversation history found for {sender}")
                                # Don't create fake messages, just skip this sender
                        except Exception as conv_error:
                            logger.warning(f"⚠️ Error getting conversation for {sender}: {conv_error}")
                            # Don't create fake messages, just skip this sender
                
                # If no messages from API, return empty list
                if not messages:
                    logger.info("ℹ️ No real messages found from ClubOS")
                    messages = []
                    
            except Exception as api_error:
                logger.warning(f"⚠️ ClubOS API error: {api_error}, returning empty messages")
                messages = []
    ...
    def get_member_conversation_history(self, member_name: str, limit=50) -> List[Dict]:
        """Get conversation history for a specific member by name"""
        try:
            from .api.enhanced_clubos_service import ClubOSAPIService
            from .authentication.secure_secrets_manager import SecureSecretsManager
            ...
            if not username or not password:
                logger.warning("⚠️ ClubOS credentials not found for conversation history")
                return []
            
            # Initialize ClubOS API service
            clubos_service = ClubOSAPIService(username, password)
            
            # Get conversation for the specific member
            conversation = clubos_service.scrape_conversation_for_contact(member_name)
            
            if not conversation:
                logger.info(f"ℹ️ No conversation history found for {member_name}")
                return []
            ...
            for i, msg in enumerate(conversation[-limit:]):
                message_data = {
                    'id': f"clubos_{member_name.replace(' ', '_')}_{i}",
                    'member_name': member_name,
                    'message_content': msg.get('content', 'No message content'),
                    'timestamp': msg.get('timestamp', datetime.now().isoformat()),
                    'sender_type': 'member' if msg.get('sender') != 'j.mayo' else 'staff',
                    'channel': 'ClubOS',
                    'status': 'received' if msg.get('sender') != 'j.mayo' else 'sent',
                    'unread': False  # Historical messages are not unread
                }
                messages.append(message_data)
            
            logger.info(f"✅ Retrieved {len(messages)} messages for {member_name}")
            return messages
