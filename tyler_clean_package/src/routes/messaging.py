...
def send_campaign():
    ...
    try:
        ...
        # Check if specific member IDs were selected (new approach)
        ...
        if selected_member_ids:
            # Use selected member IDs (new checkbox selection approach)
            member_categories = []
            logger.info(f"📋 Using selected member IDs: {len(selected_member_ids)} members selected")
        else:
            # Use category-based selection (legacy approach)
            ...
            if not member_categories:
                return jsonify({'success': False, 'error': 'At least one member category must be selected or members must be individually selected'}), 400
        ...
        # Get members from selected categories or selected member IDs with validation
        member_ids = []
        validated_members = []
        
        if selected_member_ids:
            # Handle selected member IDs (new checkbox approach)
            logger.info(f"🔍 Processing {len(selected_member_ids)} selected member IDs")
            
            # Get member data for selected IDs
            for member_id in selected_member_ids:
                member_data = current_app.db_manager.get_member_by_id(member_id)
                if member_data:
                    member_dict = {
                        'member_id': member_data.get('prospect_id') or str(member_data.get('id', '')),
                        'prospect_id': member_data.get('prospect_id'),
                    ...
                    member_id_val = member_dict['member_id']
                    if member_id_val and member_id_val not in member_ids:  # Avoid duplicates
                        member_ids.append(member_id_val)
                        validated_members.append(member_dict)
                else:
                    logger.warning(f"⚠️ Member ID {member_id} not found in database")
        else:
            # Handle category-based selection (legacy approach)
        ...
        # Send the campaign using the OPTIMIZED client with concurrent processing
        # Preserves exact API call sequence but runs multiple workers in parallel
        max_workers = int(request.form.get('max_workers', 5))  # Allow user to configure workers
        max_workers = min(max_workers, 15)  # Cap at 15 for safety
        
        results = client.send_bulk_campaign_optimized(
        ...
        results['method'] = 'optimized_concurrent'
        results['workers_used'] = max_workers
...
@messaging_bp.route('/api/messaging/inbox', methods=['GET'])
def get_inbox_messages():
    """Get all inbox messages for display"""
    try:
        # Get all messages from database
        messages = current_app.db_manager.execute_query('''
            SELECT 
                id, content, from_user, owner_id, created_at, channel,
                delivery_status, member_id, message_actions, is_confirmation,
                is_opt_in, is_opt_out, has_emoji, emoji_reactions, conversation_id
            FROM messages 
            ORDER BY created_at DESC
            LIMIT 100
        ''')
        ...
        for msg in messages:
            formatted_messages.append({
                'id': msg['id'],
                'member_name': msg['from_user'] or 'Unknown',
                'message_content': msg['content'] or 'No content',
                'timestamp': msg['created_at'].isoformat() if msg['created_at'] else datetime.now().isoformat(),
                'channel': msg['channel'] or 'ClubOS',
                'status': msg['delivery_status'] or 'received',
                'unread': msg['delivery_status'] == 'received'  # Mark as unread if just received
            })
...
@messaging_bp.route('/api/messaging/member/<member_id>/history', methods=['GET'])
# @require_auth  # Temporarily disabled for testing
