        ...
        def background_refresh():
            try:
                data_refresh_status.update({
                    ...
                    'status': 'running',
                    'message': 'Running enhanced startup sync with latest billing data...',
                    'error': None
                })
                
                # Use enhanced startup sync for comprehensive data refresh
                from src.services.multi_club_startup_sync import enhanced_startup_sync
                
                logger.info("🔄 Starting enhanced startup sync for data refresh...")
                sync_results = enhanced_startup_sync(current_app._get_current_object())
                ...
                if sync_results.get('success', False):
                    ...
                    # Get past due members count
                    past_due_members = db_manager.execute_query("""
                        SELECT COUNT(*) as count, COALESCE(SUM(amount_past_due), 0) as total
                        FROM members 
                        WHERE status_message LIKE '%Past Due%'
                    """)
                    
                    past_due_count = past_due_members[0]['count'] if past_due_members else 0
                    total_past_due = float(past_due_members[0]['total']) if past_due_members else 0.0
                    
                    data_refresh_status.update({
                        'status': 'completed',
                        'message': 'Enhanced startup sync completed successfully',
                        'completed_at': datetime.now().isoformat(),
                        'progress': 100,
                        'counts': {
                            'members': member_count,
                            'prospects': prospect_count,
                            'members_with_past_due': past_due_count,
                            'total_past_due_amount': total_past_due
                        },
                    ...
                    data_refresh_status.update({
                        'status': 'failed',
                        'message': 'Enhanced startup sync failed',
                        'completed_at': datetime.now().isoformat(),
                        'error': sync_results.get('errors', ['Unknown error'])
                    })
    ...
    try:
        ...
        def background_refresh():
            try:
                ...
                # Import fresh ClubHub data (temporarily disabled)
                # from utils.data_import import import_fresh_clubhub_data
                # import_fresh_clubhub_data()
                logger.info("🔄 ClubHub data import temporarily disabled")
                
                # Apply staff designations after sync to preserve staff status
                from utils.staff_designations import apply_staff_designations
...
def api_refresh_members():
    ...
    try:
        ...
        if success:
            ...
            # from utils.staff_designations import apply_staff_designations
            # staff_success, staff_count, staff_message = apply_staff_designations()
            logger.info("🔄 Staff designation restoration temporarily disabled")
            logger.info(f"🔄 Staff designation restoration: {staff_message}")
...
def api_refresh_billing_data():
    """API endpoint to refresh billing data from ClubHub using enhanced startup sync."""
    try:
        logger.info("🔄 Starting billing data refresh from ClubHub using enhanced startup sync...")
        
        # Use enhanced startup sync for comprehensive billing data refresh
        from src.services.multi_club_startup_sync import enhanced_startup_sync
        
        # Run the enhanced startup sync
        sync_results = enhanced_startup_sync(current_app._get_current_object())
        
        if sync_results.get('success', False):
            # Get updated billing statistics
            db_manager = current_app.db_manager
            
            # Get total members
            total_members = db_manager.get_member_count()
            
            # Get past due members with detailed billing info
            past_due_query = """
                SELECT 
                    COUNT(*) as past_due_count,
                    COALESCE(SUM(amount_past_due), 0) as total_past_due,
                    COALESCE(SUM(late_fees), 0) as total_late_fees
                FROM members 
                WHERE amount_past_due > 0
            """
            
            past_due_data = db_manager.execute_query(past_due_query)
            past_due_count = past_due_data[0]['past_due_count'] if past_due_data else 0
            total_past_due_amount = float(past_due_data[0]['total_past_due']) if past_due_data else 0.0
            total_late_fees = float(past_due_data[0]['total_late_fees']) if past_due_data else 0.0
            
            # Get status message breakdown
            status_query = """
                SELECT 
                    status_message,
                    COUNT(*) as count
            ...
            status_breakdown = db_manager.execute_query(status_query)
            
            summary = {
                'success': True,
                'message': 'Billing data refreshed successfully from ClubHub via enhanced startup sync',
                'updated_members': total_members,
                'past_due_members': past_due_count,
            ...
            logger.info(f"✅ Enhanced billing refresh complete: {past_due_count} past due members, ${total_past_due_amount:.2f} total")
            return jsonify(summary)
        else:
            return jsonify({
                'success': False,
                'error': f'Enhanced startup sync failed: {sync_results.get("errors", ["Unknown error"])}'
            }), 500
...
@api_bp.route('/member-conversation-history/<member_name>')
def get_member_conversation_history(member_name):
    ...
    try:
        ...
        # Get conversation history from ClubOS
        messages = db_manager.get_member_conversation_history(member_name, limit=50)
        ...
        logger.error(f"❌ Error getting conversation history for {member_name}: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
