...
def select_clubs():
    ...
    try:
        ...
        # Start automated access monitoring after club selection
        try:
            if hasattr(current_app, 'start_monitoring') and current_app.start_monitoring:
                current_app.start_monitoring()
                logger.info("🔐 Automated access monitoring started after club selection")
            else:
                logger.warning("⚠️ Automated access monitoring startup function not available")
        except Exception as monitoring_error:
            logger.warning(f"⚠️ Failed to start automated access monitoring after club selection: {monitoring_error}")
