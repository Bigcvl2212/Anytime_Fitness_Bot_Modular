...
def check_database_connection():
    ...
    try:
        ...
        if hasattr(current_app, 'db_manager'):
            ...
            # Handle database-specific table counting
            if current_app.db_manager.db_type == 'postgresql':
            ...
            current_app.db_manager.close_connection(conn)
            
            # Prepare details based on database type
            details = {
                'database_type': current_app.db_manager.db_type,
                'table_count': table_count
            }
            
            # Add database-specific details
            if current_app.db_manager.db_type == 'sqlite':
                details['database_path'] = current_app.db_manager.db_path
            else:
                details['database_host'] = current_app.db_manager.db_host
                details['database_name'] = current_app.db_manager.db_name
