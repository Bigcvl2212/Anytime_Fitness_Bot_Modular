...
class SecureAuthService:
    ...
    def authenticate_manager(self, clubos_username: str, clubos_password: str,
        ...
        stored_creds = self.secrets_manager.get_credentials(manager_id)
        
        if not stored_creds:
            logger.warning(f"⚠️ No stored credentials found for manager {manager_id}")
            return False, "", "No stored credentials found. Please register first."
        ...
        if (stored_creds['clubos_username'] == clubos_username and
            ...
            logger.info(f"✅ Manager {manager_id} authenticated successfully")
            return True, manager_id, ""
        else:
